...

{
				...
				// 			$price = 1;
	               ...
	               // die;
					...
					if($types=='high'){
						...
						$price = floor(($percentage/100) * round($pricePlatinum - ($pricePlatinum * 0.30)));
						$priceTotal = ($price * $weight);	
						}else{
						$pricePlatinum = round($rtiPT + ($rtiPT * -($PTpresentasePotonganPt/100)));
						$price = round(($percentage/100) * round($pricePlatinum - ($pricePlatinum * 0.30)));
						$priceTotal = round($price * $weight);	
						}
					}else{
						if($percentage<100){
						
							$pricePlatinum = floor($rtiPT + ($rtiPT * -($PTpresentasePotonganPtLow/100)));
				// 			$price = floor(($percentage/100) * round($pricePlatinum - ($pricePlatinum * 0.30)));
    ...
    function buyCheckout(){
		...
		if ($authUser == true) {
			...
			$year = date('Y',strtotime($this->dateToday));
			$noOrder = $this->TransactionModel->lastData($year)->row('t_id');
			if(!empty($noOrder)){
